...
interface OwnerRestaurantDetailsPageProps {
...
export default async function OwnerRestaurantDetailsPage({
  params,
  searchParams,
}: OwnerRestaurantDetailsPageProps) {
  const [{ id }, query] = await Promise.all([params, searchParams]);
  const [restaurant, userInfo] = await Promise.all([getRestaurantById(id), getUserInfo()]);

  if (!restaurant || !userInfo?.id || restaurant.createdByUserId !== userInfo.id) {
    notFound();
  }
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-foreground md:text-3xl">My Restaurant Details</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Detailed performance and content view for your restaurant.
          </p>
        </div>

        <Button asChild variant="outline">
          <Link href="/owner/dashboard/restaurants" className="inline-flex items-center gap-2">
