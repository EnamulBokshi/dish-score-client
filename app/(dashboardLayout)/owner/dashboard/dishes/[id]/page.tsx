...
interface OwnerDishDetailsPageProps {
...
export default async function OwnerDishDetailsPage({
  params,
  searchParams,
}: OwnerDishDetailsPageProps) {
  const [{ id }, query] = await Promise.all([params, searchParams]);
  const [dish, myRestaurantsResponse] = await Promise.all([
    getDishById(id),
    getMyRestaurants("limit=200"),
  ]);
  ...
  const ownedRestaurantIds = new Set((myRestaurantsResponse.data ?? []).map((r) => r.id));
  const dishRestaurantId = dish.restaurant?.id || dish.restaurantId;

  if (!dishRestaurantId || !ownedRestaurantIds.has(dishRestaurantId)) {
    notFound();
  }
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-foreground md:text-3xl">My Dish Details</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Detailed analytics and feedback for your dish.
          </p>
        </div>

        <Button asChild variant="outline">
          <Link href="/owner/dashboard/dishes" className="inline-flex items-center gap-2">
