...
interface AdminDishManagementTableProps {
  queryString: string;
...
export default function AdminDishManagementTable({
  queryString,
  queryParamsObject,
}: AdminDishManagementTableProps) {
  ...
  const { data: restaurantsResponse } = useQuery({
    queryKey: ["admin-restaurants-common"],
    queryFn: () => getRestaurants("limit=200"),
  });
  ...
  const { mutateAsync: updateDishMutation, isPending: isUpdatingDish } = useMutation({
    mutationFn: ({
      ...
      image?: File;
    }) => updateDishByAdmin(dishId, payload, image),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["admin-dishes"] });
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <DataTable<IDish>
          ...
          filters={
            <div className="flex items-center gap-2">
              ...
              <DataTableFilterPopover
                ...
                title="Filter dishes"
                description="Filter by restaurant, price, and average rating."
              >
          ...
          actions={{
            onView: (dish) => router.push(`/admin/dashboard/dishes/${dish.id}`),
        ...
        <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
          <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>
                Add a new dish with ingredients, pricing, and image.
              </DialogDescription>
            </DialogHeader>
            <CreateDishForm
